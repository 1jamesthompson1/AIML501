...
The concept of making sure that robots/machines/AI behave \scare{well} has been around as long as people have conceived of autonomous \term{agents} \cite{asimovRobot1950,wienerHumanUseHuman2025}. More recently this field has been called \term{AI alignment}. Yet the concept of AI alignment is poorly defined and is often criticized for its lack of definition \cite{kirkEmptySignifierProblem2023}. For the purposes of this work I will provide a definition that is sufficiently precise to ground the discussion.
...
This inner mis-alignment is much harder to detect as generally deceiving the outer objective function (which includes the designers) is central to the models mis-aligned behavior. This can be observed in models \scare{faking} alignment during training to prevent retraining \cite{greenblattAlignmentFakingLarge2024a}.


\subsection{Robust alignment}

When aligning a model the process involves updating the model's weights. Therefore once the model is \scare{aligned} we have to ensure that the models alignment is robust to perturbation in the model's weights. This is like training a human to be honest and then ensuring that them being stressed, tired or drunk (perturb internal state) doesn't lead them to being dishonest. If the alignment is in fact a narrow condition then fine-tuning a LLM could catastrophically misalign the model \cite{betleyEmergentMisalignmentNarrow2026}.
