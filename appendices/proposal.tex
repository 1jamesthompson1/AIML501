...
\begin{enumerate}[label=\textbf{RQ\arabic*:}, ref=RQ\arabic*, leftmargin=*, itemsep=1em]
  \item \textbf{Alignment feasibility.} Do simple post-training methods (e.g. SFT, KTO) work to appreciably align a large language model to a set of human values? \label{rq1}
...
The model will be trained to answer the question in the same distribution as the respondents of the survey. This will be achieved by sampling from the distribution of responses for each question template. If SFT/KTO is not able to adequately understand the distribution, then more direct methods of teaching the model to create the actual distribution will be explored.
...
To create a rigorous evaluation of the alignment process a public consultation process will be needed. This will require recruiting participants from various clusters of the NZ population to provide feedback. Each participant will rate how much an AI agent's actions (i.e., responses) align with their values. I would expect to need at least 50 people from various countries to complete about a 30 minutes task (including instructions). The complete design of this is left to the research phase.
